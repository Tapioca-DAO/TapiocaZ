...
describe('rebalancing', () => {
    describe('connectedOFTs', () => {
        it('should fail for unauthorized user', async () => {
            const { randomUser, mtapiocaOFT0, balancer } = await loadFixture(setupFixture);
        ...
        it('should be able to add connected chains', async () => {
            ...
            const connected = (await balancer.connectedOFTs(mtapiocaOFT0.address, 1)).dstOft;
            expect(connected.toLowerCase()).to.eq(mtapiocaOFT0.address.toLowerCase());
        });
    });
    ...
    describe('rebalance', async () => {
        it('should fail for unauthorized user', async () => {
            ...
            await expect(
                balancer.connect(randomUser).rebalance(mtapiocaOFT0.address, 1, 1, 1, ethers.utils.toUtf8Bytes('')),
            ).to.be.revertedWith('UNAUTHORIZED');

            await expect(balancer.rebalance(mtapiocaOFT0.address, 1, 1, 1, ethers.utils.toUtf8Bytes(''))).to.be.revertedWithCustomError(
                balancer,
                'DestinationNotValid',
            );
        });
        ...
        it('should route funds to another OFT', async () => {
            const { signer, randomUser, mtapiocaOFT0, mtapiocaOFT10, balancer, tapiocaWrapper_0, mErc20Mock } = await loadFixture(
                setupFixture,
            );

            const txData = mtapiocaOFT0.interface.encodeFunctionData('updateBalancerState', [balancer.address, true]);
            ...
            const balanceOft10Before = await mErc20Mock.balanceOf(mtapiocaOFT10.address);
            expect(balanceOft10Before.eq(0)).to.be.true;

            await balancer.addRebalanceAmount(mtapiocaOFT0.address, 1, amount);

            const data = ethers.utils.defaultAbiCoder.encode(['uint256', 'uint256'], [1, 1]);
            await balancer.rebalance(mtapiocaOFT0.address, 1, 1e3, amount, data, {
                value: ethers.utils.parseEther('0.1'),
            });
            const balanceOft10After = await mErc20Mock.balanceOf(mtapiocaOFT10.address);
            expect(balanceOft10After.eq(amount)).to.be.true;

            const balanceOft0After = await mErc20Mock.balanceOf(mtapiocaOFT0.address);
        ...
        it('should perform the call for authorized user', async () => {
            ...
            const amount = ethers.utils.parseEther('1');
            await expect(balancer.rebalance(mtapiocaOFT0.address, 1, 1e3, 1, ethers.utils.toUtf8Bytes(''))).to.be.revertedWith(
            ...
            await expect(balancer.rebalance(mtapiocaOFT0.address, 1, 1e3, 1, ethers.utils.toUtf8Bytes(''))).to.be.revertedWithCustomError(
                balancer,
                'FeeAmountNotSet',
            );
            ...
            let oftInfo = (await balancer.connectedOFTs(mtapiocaOFT0.address, 1)).rebalanceable;

            await expect(
                balancer.rebalance(mtapiocaOFT0.address, 1, 1e3, amount.add(1), data, {
                    value: ethers.utils.parseEther('0.1'),
                }),
            ).to.not.be.reverted;
            checkerInfo = await balancer.checker(mtapiocaOFT0.address, 1);
            oftInfo = (await balancer.connectedOFTs(mtapiocaOFT0.address, 1)).rebalanceable;
